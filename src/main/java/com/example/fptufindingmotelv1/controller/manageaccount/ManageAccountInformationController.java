...
import com.example.fptufindingmotelv1.service.manageaccount.ManageAccountInformationService;
...
@Controller
public class ManageAccountInformationController {

    @Autowired
    private ManageAccountInformationService manageAccountInformationService;
    ...
    public JSONObject saveUserInfo(@RequestBody UserDTO request){
        ...
        if(manageAccountInformationService.saveUserInfo(request)){
    ...
    public JSONObject savePhoneNumber(@RequestBody UserDTO request){
        JSONObject response = new JSONObject();
        if(manageAccountInformationService.savePhone(request)){
            response.put("msgCode", "user000");
    ...
    public JSONObject savePassword(@RequestBody UserDTO request){
        return manageAccountInformationService.savePassword(request);
    }
}
