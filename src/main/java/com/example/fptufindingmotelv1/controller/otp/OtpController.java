...
public class OtpController {
    ...
    public JSONObject sendOTP(@RequestParam String phoneNumber, @RequestParam String siteCode) {
        boolean existPhone = userRepository.existsByPhoneNumber(phoneNumber);
        if (siteCode.equals(env.getProperty("ffm.sms.siteCode.forgotPassword"))) {
            if (existPhone) {
        ...
        } else if (siteCode.equals(env.getProperty("ffm.sms.siteCode.register"))) {
            if (existPhone) {
                return responseMsg("001", "Số điện thoại này đã tồn tại.", null);
            } else {
                char[] otp = otpService.generateOTP(phoneNumber);
                return otpService.sendSms(phoneNumber, otp);
            }
        } else {
            return responseMsg("999", "Lỗi tạo mã OTP.", null);
        }
    }
